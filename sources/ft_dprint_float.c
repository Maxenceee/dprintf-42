...
static void	ft_dprint_in_float_spaces(char *d_i, double save_f,
	t_data *data, t_flags flags)
{
	...
	if (flags.dot >= 0)
		ft_dprint_max_width(flags.dot,
			(int)ft_strlen_floating_part(d_i) - 1, 1, data);
}

static void	ft_dprint_float_spaces(char *d_i, double save_f,
	t_data *data, t_flags flags)
{
	if (flags.minus == 1)
		ft_dprint_in_float_spaces(d_i, save_f, data, flags);
	if (flags.dot >= 0 && (size_t)flags.dot < ft_strlen(d_i))
		flags.dot = ft_strlen(d_i);
	if (flags.dot >= 0)
	{
		flags.width -= flags.dot;
		ft_dprint_width(flags.width, 0, flags.zero, data);
	}
	else
		ft_dprint_width(flags.width, ft_strlen(d_i), flags.zero, data);
	if (flags.minus == 0)
		ft_dprint_in_float_spaces(d_i, save_f, data, flags);
}

static void	ft_dprint_float_flags(t_flags *flags, double *li, t_data *data)
{
...
void	ft_dprint_float(double i, t_data *data, t_flags flags)
{
	...
	if (flags.dot >= 0)
		max = flags.dot;
	ft_dprint_float_flags(&flags, &li, data);
	d_i = ft_ftoa(li, max);
	if (!d_i)
		return ;
	ft_dprint_float_spaces(d_i, i, data, flags);
	free(d_i);
}
