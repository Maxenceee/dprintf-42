...
static void	ft_dprint_in_int_spaces(char *d_i, int save_i,
	t_data *data, t_flags flags)
{
	if (save_i < 0 && flags.plus == 0 && flags.dot >= 0)
		ft_dputchar_c('-', data);
	if (flags.dot >= 0)
		ft_dprint_width(flags.dot - 1, ft_strlen(d_i) - 1, 1, data);
	ft_putstrprec(d_i, ft_strlen(d_i), data);
}

static void	ft_dprint_int_spaces(char *d_i, int save_i,
	t_data *data, t_flags flags)
{
	if (flags.minus == 1)
		ft_dprint_in_int_spaces(d_i, save_i, data, flags);
...
static void	ft_dprint_int_flags(t_flags *flags, long int *li, t_data *data)
{
...
void	ft_dprint_int(int i, t_data *data, t_flags flags)
{
	...
	ft_dprint_int_flags(&flags, &li, data);
	d_i = ft_itoa(li);
	if (!d_i)
		return ;
	ft_dprint_int_spaces(d_i, i, data, flags);
	free(d_i);
}
